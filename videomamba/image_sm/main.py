...
def get_args_parser():
    ...
    parser.add_argument('--load_from_distill', default=None, type=str, metavar='MODEL',
                        help='Name of model to train')
...
def main(args):
    ...
    if args.load_from_distill is not None:
        ...
        for k in checkpoint['model'].keys():
            if 'distill_head' in k:
                delete_group.append(k)
        for k in delete_group:
            del checkpoint['model'][k]
            if 'model_ema' in checkpoint:
                del checkpoint['model_ema'][k]
        model_without_ddp.load_state_dict(checkpoint['model'])
        if not args.eval and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint:
            print("Du not load optimizer")
