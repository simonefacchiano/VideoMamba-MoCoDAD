...
from engine_distill import train_one_epoch, evaluate, evaluate_ema
...
def get_args_parser():
    ...
    parser.add_argument('--teacher_model', default='deit_base_patch16_224', type=str, metavar='MODEL',
                        help='Name of model to train')
    parser.add_argument('--teacher_embed_dim',  default=384, type=int, metavar='MODEL',
                        help='Embedding dimenstion of teacehr model')
    parser.add_argument('--teacher_pretrained_path',  default=None, type=str, metavar='MODEL',
                        help='Pretrained model path for teacher model')
    ...
    # Dataset parameters
    parser.add_argument('--root_dir_train', default='your_imagenet_path/train/', type=str, help='dataset path')
    parser.add_argument('--meta_file_train', default='your_imagenet_path/meta/train.txt', type=str, help='dataset path')
    parser.add_argument('--root_dir_val', default='your_imagenet_path/val/', type=str, help='dataset path')
    parser.add_argument('--meta_file_val', default='your_imagenet_path/meta/val.txt', type=str, help='dataset path')
    parser.add_argument('--data-path', default='your_imagenet_path/', type=str,
                        help='dataset path')
...
def main(args):
    ...
    assert args.teacher_pretrained_path is not None, "teacher_pretrained_path is None!!!"
    state_dict = torch.load(args.teacher_pretrained_path, "cpu")
    if "model" in state_dict.keys():
        state_dict = state_dict["model"]
    print(f"Load teacehr model from {args.teacher_pretrained_path}")
    ...
    if args.finetune:
        ...
        if orig_size != new_size:
            print(f"Interpolate position embedding from {orig_size} to {new_size}")
    ...
    if args.if_amp:
        ...
        teacher_model = teacher_model.half() if args.bf16 else teacher_model.to(torch.bfloat16)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            optimizer, device, epoch, loss_scaler, amp_autocast, teacher_model,
